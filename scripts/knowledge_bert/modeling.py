...
# changed it from ernie_config.json
...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        config_ent.hidden_size = ENTITY_EMBED # 100
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense_ent = nn.Linear(ENTITY_EMBED, config.intermediate_size)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense_ent = nn.Linear(config.intermediate_size, ENTITY_EMBED)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.LayerNorm_ent = BertLayerNorm(ENTITY_EMBED, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertEntPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        config_ent = copy.deepcopy(config)
        config_ent.hidden_size = ENTITY_EMBED
...
class ErnieForMaskedLMEmrQA(PreTrainedBertModel):
    """BERT model with the masked language modeling head.
    This module comprises the BERT model followed by the masked language modeling head.
    ...
    ```
    """
    def __init__(self, config, entity_num=20):
        super(ErnieForMaskedLMEmrQA, self).__init__(config)
        self.bert = BertModel(config)
        self.entity_embeds = nn.Embedding(entity_num, ENTITY_EMBED)
        self.cls = BertOnlyMLMHead(config, self.bert.embeddings.word_embeddings.weight)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, input_ents_idx, ent_mask=None, token_type_ids=None, attention_mask=None, masked_lm_labels=None):
        input_ents = self.entity_embeds(input_ents_idx)
        print(input_ents)
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, input_ents, ent_mask,
                                       output_all_encoded_layers=False)
        prediction_scores = self.cls(sequence_output)
...
class ErnieForMaskedLMEmrQASentenceClassification(PreTrainedBertModel):
    """BERT model with the masked language modeling head.
    This module comprises the BERT model followed by the masked language modeling head.
    ...
    ```
    """
    def __init__(self, config, num_labels=2, entity_num=20):
        super(ErnieForMaskedLMEmrQASentenceClassification, self).__init__(config)
        # Things required for the LM model
        self.bert = BertModel(config)
        self.entity_embeds = nn.Embedding(entity_num, ENTITY_EMBED)
        self.cls = BertOnlyMLMHead(config, self.bert.embeddings.word_embeddings.weight)
        # Things required for the sentence classification model
    ...
    def forward(self, input_ids, input_ents_idx, ent_mask=None, token_type_ids=None, attention_mask=None, masked_lm_labels=None, lm_flag=True, labels = None):
        # lm_flag = True while training the language model and False while learning the sentence classification task
        if(lm_flag):
            input_ents = self.entity_embeds(input_ents_idx)
            # print(input_ents)
            sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, input_ents, ent_mask,
    ...
    #             print(masked_lm_labels.view(-1).shape)
            ...
            input_ent = self.entity_embeds(input_ents_idx)
...
class BertForSequenceClassificationEntity(PreTrainedBertModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    ...
    ```
    """
    def __init__(self, config, num_labels=2, num_entities=20):
        super(BertForSequenceClassificationEntity, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.entity_embeds = nn.Embedding(num_entities, ENTITY_EMBED)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ents_idx=None, ent_mask=None, labels=None):
        input_ent = self.entity_embeds(input_ents_idx)
        _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, input_ent, ent_mask, output_all_encoded_layers=False)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
...
class BertForEmrQA(PreTrainedBertModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    ...
    ```
    """
    def __init__(self, config, num_labels=2, entity_num=39):
        super(BertForEmrQA, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        self.entity_embeds = nn.Embedding(entity_num, ENTITY_EMBED) # had to hard code the embedding size because of ERNIE code, earlier hard coded embeds
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent_idx=None, ent_mask=None, labels=None):
        input_ent = self.entity_embeds(input_ent_idx)
#         print(input_ent.shape)
        _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, input_ent, ent_mask, output_all_encoded_layers=False)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
...
class BertForEmrQAProj(PreTrainedBertModel):
    def __init__(self, config, num_labels=2, entity_num=39, num_logical_forms=30, lf_embed_size=70):
        super(BertForEmrQAProj, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        self.entity_embeds = nn.Embedding(entity_num, ENTITY_EMBED) # had to hard code the embedding size because of ERNIE code, earlier hard coded embeds
        self.logical_form_embeds = nn.Embedding(num_logical_forms, lf_embed_size)
        self.activation = nn.Tanh()
        self.dotAttnProj = DotAttentionLayerProj(lf_embed_size, config.hidden_size)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent_idx=None, ent_mask=None, logical_form_ids = None, sent_lens=None,labels=None, device=None):
        input_ent = self.entity_embeds(input_ent_idx)
        lf_embeds = self.logical_form_embeds(logical_form_ids)
        lf_embeds = lf_embeds.unsqueeze(dim=1)
#         print(input_ent.shape)
        token_output , pooled_output = self.bert(input_ids, token_type_ids, attention_mask, input_ent, ent_mask, output_all_encoded_layers=False)

        # pooled_output = self.dropout(pooled_output)
        token_output = self.dotAttnProj((token_output, sent_lens, lf_embeds, device))
...
class DotAttentionLayerProj(nn.Module):
    def __init__(self, lf_hidden_dim, hidden_dim):
        super(DotAttentionLayerProj, self).__init__()
#         self.hidden_size = hidden_size
        self.lf_hidden_dim = lf_hidden_dim
        self.hidden_dim = hidden_dim
        self.W = nn.Bilinear(lf_hidden_dim, hidden_dim, 1, bias=False)

    def forward(self, input):
        """
        input: (unpacked_padded_output: batch_size x seq_len x hidden_size, lengths: batch_size)
        """
        inputs, lengths, lf_inputs, device = input
        batch_size, max_len, _ = inputs.size()
        lf_inputs = lf_inputs.repeat((1,max_len,1))
        logits = self.W(lf_inputs, inputs)
...
class BertForEmrQAProjMultiTask(PreTrainedBertModel):
    def __init__(self, config, num_labels=2, entity_num=39, num_logical_forms=30, lf_embed_size=70):
        super(BertForEmrQAProjMultiTask, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        self.classifier_lf = nn.Linear(config.hidden_size, num_logical_forms)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent_idx=None, ent_mask=None, logical_form_ids = None, sent_lens=None,labels=None):
        input_ent = self.entity_embeds(input_ent_idx)
        lf_embeds = self.logical_form_embeds(logical_form_ids)
        lf_embeds = lf_embeds.unsqueeze(dim=1)
#         print(input_ent.shape)
        ...
        # pooled_output = self.dropout(pooled_output)
        token_output = self.dotAttnProj((token_output, sent_lens, lf_embeds))
        token_output = self.activation(token_output)
        token_output = self.dropout(token_output)
        logits = self.classifier(token_output)
        logits_lf = self.classifier_lf(token_output)
        ...
        if labels is not None:
            ...
            return logits, logits_lf

class BertForEmrQAMultiTask(PreTrainedBertModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    ...
    ```
    """
    def __init__(self, config, num_labels=2, entity_num=4, num_logical_forms=30):
        super(BertForEmrQAMultiTask, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        self.entity_embeds = nn.Embedding(entity_num, ENTITY_EMBED) # had to hard code the embedding size because of ERNIE code, earlier hard coded embeds
        self.classifier_lf = nn.Linear(config.hidden_size, num_logical_forms) # logical forms are also in a way hard coded, there is some error occurring when they are passed as variable (ToDo)
        self.apply(self.init_bert_weights)
...
#         print(input_ent.shape)
        ...
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
        logits_lf = self.classifier_lf(pooled_output)
...
class BertForQuestionAnsweringEmrQA(PreTrainedBertModel):
    def __init__(self, config, num_entities=20, num_logical_forms=30):
        super(BertForQuestionAnsweringEmrQA, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.entity_embeds = nn.Embedding(num_entities, ENTITY_EMBED)
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
        self.logical_form_outputs = nn.Linear(config.hidden_size, num_logical_forms)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ents_idx=None, ent_mask=None, start_positions=None, end_positions=None):
        input_ent = self.entity_embeds(input_ents_idx)
        ...
        pooled_output = self.dropout(pooled_output)
        logicalform_logits = self.logical_form_outputs(pooled_output)
        ...
        if start_positions is not None and end_positions is not None:
            ...
            return total_loss, logicalform_logits
        else:
            return (start_logits, end_logits), logicalform_logits

class BertForSequenceClassificationBoolQ(PreTrainedBertModel):
    def __init__(self, config, num_labels=2, num_entities=20, num_title=10):
        super(BertForSequenceClassificationBoolQ, self).__init__(config)
        self.num_labels = num_labels
        self.entity_embeds = nn.Embedding(num_entities, ENTITY_EMBED)
        self.title_outputs = nn.Linear(config.hidden_size, num_title)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent_idx=None, ent_mask=None, labels=None):
        ...
        title_logits = self.title_outputs(pooled_output)
#         print(logits.shape)
#         print(labels.shape)
#         print(title_logits.shape)
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            return loss, title_logits
        else:
            return logits, title_logits
